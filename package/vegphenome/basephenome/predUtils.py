...
from fastai.vision.all import PILImage, load_learner, tensor
...
class detectroninference:
    """
    Detection Inference Class for MaskRCNN and KeyPoint Detection
    """
    ...
    def __init__(self, model_path, num_cls=1, name_classes=["pepp"]):
        ...
        Args:
            model_path ([str]): MaskRCNN weigths for Instance segemntation
            num_cls (int, optional): Number of classes. Defaults to 1.
            name_classes (list, optional): Name of classes. Defaults to ["pepp"].
    ...
    def apply_mask(self, mask, img):
        """Get masks and patches same size as of input images from the predicted masks from Instance segemntation
    ...
    def pred(self, img):
        """Run the MaskRCNN instance segmentation on input Image

        Args:
            img ([np.array]): Input Image of dimension HxWx3

        Returns:
            [Tuple(np.array,np.array,np.array,np.array,np.array,np.array)]: Tuple of visualized detection, masks, patches, boxes, classes and scores
        """
        orig_img = img.copy()
...
def get_labels(filename):
    """Get labels for orientation correction

    Args:
        filename ([str]): File name with points annotation
    """
    points=[]
