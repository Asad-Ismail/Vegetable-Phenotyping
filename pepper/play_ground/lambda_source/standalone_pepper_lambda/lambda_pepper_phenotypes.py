...
def phenotype_measurement_pepper(img, patch,ppx=None,ppy=None):
    """Takes input mask and cropped RGB image and optional ppx and ppy
    Returns : Phenotypes, visualized images with phenotypes and labels of phenotypes
    """
...
def run_phenotype_detection(image_name,mask,fruit_patch):
    """Inputs:
    image_name: str
    input mask and cropped RGB image. Both mask and RGB image have size smaller than original image since they are detected cropped images
    Returns : Pandas dataframe of phenotypes
    
    """
        #binarize the mask
    assert mask is not None,"Mask is None"
    assert fruit_patch is not None,"Fruit Patch is None"
...
def lambda_handler(event, context):
    ...
    results_df=run_phenotype_detection(image_name,mask,fruit_patch)
    results_json = results_df.to_json(orient="index")
    logger.info('Calculated result %s', results_json)
    return results_json
